...
- (void)viewDidLoad {
    ...
    arma::fmat affine_points = myproj_affine(W, A);
    display_im = DrawPts(display_im, affine_points, GREEN);
    display_im = DrawLines(display_im, affine_points, GREEN);
    
    const cv::Scalar BLUE = cv::Scalar(255,0,0);
    arma::fmat H = myfit_homography(X, W);
    arma::fmat homo_points = myproj_homography(W, H);
    display_im = DrawPts(display_im, homo_points, BLUE);
    display_im = DrawLines(display_im, homo_points, BLUE);
