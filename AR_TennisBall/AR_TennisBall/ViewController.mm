...
- (void)viewDidLoad {
    ...
    arma::fmat E = myfit_extrinsic(K, H);
    const cv::Scalar YELLOW = cv::Scalar(0,255,255);
    arma::fmat points = myproj_extrinsic(K, E, sphere);
    //Coordinates to Move (828,1650)
    //Create the translation matrix
    arma::fmat translate_to_O;
    translate_to_O << (828-X(0,0)-40) << arma::endr
    << (1650-X(1,0)-200) << arma::endr;
    points = myproj_translate(points, translate_to_O);
    display_im = DrawPts(display_im, points, YELLOW);
    //display_im = DrawLines(display_im, points, YELLOW);
